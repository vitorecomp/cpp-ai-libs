...
void Configs::decodeMap() {
	...
	if (json["seeds"] != Json::nullValue) {
		...
		for (int index = 0; index < seeds.size();
	...
	if (json["neurons"] != Json::nullValue) {
		this->num_neurons = json["neurons"].size();
		this->types = (int *)calloc(num_neurons, sizeof(int));
		for (uint i = 0; i < json["neurons"].size(); i++) {
			this->types[i] = json["neurons"][i].asInt();
		}
	} else {
		this->num_neurons = 10;
		this->types = (int *)calloc(num_neurons, sizeof(int));
		for (uint i = 0; i < this->num_neurons; i++) {
			this->types[i] = i;
		}
	}

	if (json["threads"] != Json::nullValue)
		this->threads = json["threads"].asInt();
	else
		this->threads = 1;
}
...
void Output::makeMap() {
	Screen *main_messages = (Screen *)new MessageBox(
		"main_messages", 0, 0, 20, 80, io::configs.windows_lines);
